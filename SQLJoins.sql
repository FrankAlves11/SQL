...
ON e.EmployeeID = s.EmployeeID GROUP BY e.EmployeeID ORDER BY Quantity DESC;
...
/* joins: find the product name, total # sold, and total price sold,
 ...
 SELECT p.Name, SUM(s.Quantity) AS Quantity, SUM(s.Quantity * p.Price) AS Price
 FROM products AS p
 INNER JOIN sales AS s
